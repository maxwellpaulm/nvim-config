...
local telescope_notifications = function(opts)
  local config_names = require("user.debug.config").list_configs()
  local reversed = {}
  for i, config_name in ipairs(config_names) do
    reversed[#config_names - i + 1] = config_name
  end
  pickers
    .new(opts, {
      results_title = "Debug Configs",
      prompt_title = "Filter Configs",
...
return require("telescope").register_extension({ exports = { debug_config = telescope_notifications } })
